...
> Agent-agnostic instructions for any AI assistant working with this knowledge base.
> This file follows the [AGENTS.md convention](https://github.com/anthropics/agent-conventions) for multi-agent compatibility.
...
```
raw/          Immutable source materials. Never modify during compilation.
...
Always read `kb.yaml` at the start of any KB operation. If it doesn't exist, initialize the KB first. Respect all settings in the config — batch limits, auto-lint preferences, query depth, etc.

## KB Operations

Any AI agent working with this KB should support these core operations:

### Compile
Transform raw materials into wiki articles. Read `raw/` files, synthesize into structured articles in `wiki/`, regenerate `_index.md` and `_glossary.md`, run link checks.

### Query
Answer questions against the knowledge base. Read `wiki/_index.md` first to route to relevant articles, then read full articles. Save answers to `output/`.

### Research
Deep multi-step investigation on a topic. Formulate sub-questions, search wiki and raw materials, identify gaps, produce structured report in `output/`. Optionally file findings back to `raw/` for future compilation.

### Lint
Health check the KB for broken wikilinks, orphan articles, stale content, and coverage gaps.
...
1. **Research** → writes report to `output/`
2. **File back** → copy relevant output to `raw/` for future compilation
3. **Compile** → new raw material becomes wiki articles
4. **Query** → queries against richer wiki, produces better answers
